...
from .utils import is_document_in_present_data
...
class DeletionSyncCommand(BaseCommand):
    ...
    def remove_deleted_documents_from_global_keys(
        ...
        for item in list_ids_documents:
            ...
            items_exists = list(
                filter(
                    lambda seq: is_document_in_present_data(seq, item_id),
                    live_documents,
                )
            )
            if (
                len(items_exists) == 0
                and self.config.get_value("connector_platform_type") in platform
            ):
    ...
    def create_jobs_for_mails_deletion(
        ...
        :param sync_microsoft_outlook: Object for fetching the Microsoft Outlook object
        ...
        mails_documents = self.create_jobs(
            ...
            ([], users_accounts, self.microsoft_outlook_mail_object, True),
            time_range_list,
        )
    ...
    def create_jobs_for_calendar_deletion(
        ...
        calendar_documents = self.create_jobs(
            ...
            ([], users_accounts, self.microsoft_outlook_calendar_object, True),
            time_range_list,
        )
        delete_keys_documents = storage_with_collection.get("delete_keys") or []
        global_keys_documents = storage_with_collection.get("global_keys") or []
    ...
    def create_jobs_for_contacts_deletion(
        ...
        contacts_documents = self.create_jobs(
            ...
            ([], users_accounts, self.microsoft_outlook_contact_object, True),
            time_range_list,
        )
        delete_keys_documents = storage_with_collection.get("delete_keys") or []
        global_keys_documents = storage_with_collection.get("global_keys") or []
    ...
    def create_jobs_for_tasks_deletion(
        ...
        tasks_documents = self.create_jobs(
            ...
            ([], users_accounts, self.microsoft_outlook_task_object, True),
            time_range_list,
        )
        delete_keys_documents = storage_with_collection.get("delete_keys") or []
        global_keys_documents = storage_with_collection.get("global_keys") or []
    ...
    def start_producer(self, queue):
        ...
        _, time_range_list = self.get_datetime_iterable_list_based_on_full_inc_sync(
            "", ""
        )
