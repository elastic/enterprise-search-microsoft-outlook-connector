...
class Checkpoint:
    ...
    def get_checkpoint(self, current_time, obj_type):
        ...
        if (
            ...
            self.logger.info(
                "Checkpoint file exists and has contents, hence considering the checkpoint time instead of \
            ...
            self.logger.debug(
                f"Checkpoint file does not exist at {self.CHECKPOINT_PATH}, considering the start_time and end_time from \
    ...
    def set_checkpoint(self, current_time, index_type, obj_type):
        ...
        try:
            ...
            self.logger.debug(
                ...
                the checkpoint path: {self.CHECKPOINT_PATH}"
            )
        ...
        with open(self.CHECKPOINT_PATH, "w", encoding="UTF-8") as checkpoint_store:
            try:
                ...
                self.logger.exception(
                    f"Error while updating the existing checkpoint json file. Adding the new \
                    content directly instead of updating. Error: {exception}"
                )
