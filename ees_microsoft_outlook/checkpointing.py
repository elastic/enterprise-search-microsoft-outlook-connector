...
class IncorrectFormatError(Exception):
    ...
    def __init__(self, obj_type, checkpoint, inner_exception, CHECKPOINT_PATH):
        super().__init__(
            ...
            f"the correct format. Expected format: {constant.RFC_3339_DATETIME_FORMAT}. Remove the checkpoint entry "
...
class Checkpoint:
    ...
    def get_checkpoint(self, current_time, obj_type):
        ...
        """
        self.logger.info(f"Fetching the checkpoint details for {obj_type} from the checkpoint file: {self.CHECKPOINT_PATH}")
        ...
        if os.path.exists(self.CHECKPOINT_PATH) and os.path.getsize(self.CHECKPOINT_PATH) > 0:
            self.logger.info("Checkpoint file exists and has contents, hence considering the checkpoint time instead of start_time and end_time")
                    ...
                    if not checkpoint_list.get(obj_type):
                        ...
                        try:
                            ...
                            if start_time >= constant.CURRENT_TIME:
                                raise Exception("Checkpoint file contain greater time than current time")
        ...
        self.logger.debug(f"Contents of the start_time: {start_time} and end_time: {end_time} for {obj_type}")
    ...
    def set_checkpoint(self, current_time, index_type, obj_type):
        ...
        try:
            ...
            if isinstance(exception, FileNotFoundError):
                self.logger.debug(f"Checkpoint file not found on path: {self.CHECKPOINT_PATH}. Generating the checkpoint file")
            else:
                self.logger.exception(
